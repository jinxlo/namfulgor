...
def receive_data():
    ...
    inserted_count = 0
    updated_count = 0
    skipped_no_change_count = 0   # <-- New: count for skipped items
    ...
    with db_utils.get_db_session() as session:
        ...
        for i, product_entry_data in enumerate(damasco_payload):
            ...
            try:
                ...
                embedding_vector = generate_product_embedding(text_for_embedding)
            except Exception as e:
            ...
            # 3. Add or Update Product-Location Entry in DB (handle skip logic)
            try:
                ...
                if success:
                    if op_message == "added":
                        ...
                        updated_count += 1
                    elif op_message == "skipped_no_change":   # <-- New: handle no-change
                        skipped_no_change_count += 1
                        logger.info(f"{log_prefix} DB: Entry unchanged, skipped update.")
                    else:
    ...
    total_failed_or_skipped = (
        skipped_no_text_for_embedding_count +
    ...
    summary = {
        ...
        "skipped_no_change": skipped_no_change_count,    # <-- New: show skipped in API response
